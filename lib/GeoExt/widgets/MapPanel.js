...
GeoExt.MapPanel = Ext.extend(Ext.Panel, {
    ...
    initComponent: function(){
        if(!(this.map instanceof OpenLayers.Map)) {
			...
			this.map.events.register("addlayer", undefined, function(e) {				
				
				e.layer.events.register("loadstart", e.layer, function(a) {
					Ext.getCmp('waitIndicator').show();
					setTimeout(function(){Ext.getCmp('waitIndicator').hide()},1000);
					gxp.plugins.Logger.log("Отправлен запрос на получение участка карты \"" + a.object.name + "\" к серверу " + a.object.url,
											gxp.plugins.Logger.prototype.LOG_LEVEL_INFO);
				});

				e.layer.events.register("tileloaded", e.layer, function(a) {
					Ext.getCmp('waitIndicator').show();
					setTimeout(function(){Ext.getCmp('waitIndicator').hide()},1000);
					gxp.plugins.Logger.log("Получен участок карты \"" + a.object.name + "\" от сервера " + a.object.url+ " Осталось: "+ a.object.numLoadingTiles,
											gxp.plugins.Logger.prototype.LOG_LEVEL_INFO);
				});

				e.layer.events.register("loadend", e.layer, function(a) {
					Ext.getCmp('waitIndicator').show();
					setTimeout(function(){Ext.getCmp('waitIndicator').hide()},1000);
					gxp.plugins.Logger.log("Завершена загрузка карты \"" + a.object.name + "\" с сервера " + a.object.url,
											gxp.plugins.Logger.prototype.LOG_LEVEL_INFO);
				});
				
				e.layer.events.register("loaderror", e.layer, function(a) {					
					Ext.getCmp('waitIndicator').show();
					setTimeout(function(){Ext.getCmp('waitIndicator').hide()},1000);
					gxp.plugins.Logger.log("Произошла ошибка при загрузке карты \"" + a.object.name + "\" с сервера " + a.object.url,
											gxp.plugins.Logger.prototype.LOG_LEVEL_INFO);
				});

			});
			OpenLayers.Util.IMAGE_RELOAD_ATTEMPTS = 3;
        }
    ...
    onChangelayer: function(e) {
        if(e.property) {
            if(e.property === "visibility") {
                this.fireEvent("afterlayervisibilitychange", e);

...
GeoExt.data.PrintProvider = Ext.extend(Ext.util.Observable, {
    ...
    dpis: null,
     ...
     /** api: property[formats]
     *  ``Ext.data.JsonStore`` read-only. A store representing the formats
     *  available.
     *  
     *  Fields of records in this store:
     *  
     *  * name - ``String`` the name of the format
     *  * value - ``String`` the dots per inch
     */
    formats: null,
    ...
    /** api: property[format]
     *  ``Ext.data.Record`` the record for the currently used resolution.
     *  Read-only, use ``setFormat`` to set the value.
     */
    format: null,
    ...
    constructor: function(config) {
        ...
        this.addEvents(
            ...
            /** api: event[formatchange]
             *  Triggered when the format value is changed.
             *  
             *  Listener arguments:
             *
             *  * printProvider - :class:`GeoExt.data.PrintProvider` this
             *    PrintProvider
             *  * format - ``Ext.data.Record`` the new format record
             */
            "formatchange",
        ...
        this.formats = new Ext.data.JsonStore({
            root: "outputFormats",
            fields: ["name"]
        });
    ...
    /** api: method[setFormat]
     *  :param format: ``Ext.data.Record`` the format record.
     *  
     *  Sets the format for this printProvider.
     */
    setFormat: function(format) {
        this.format = format;
        this.fireEvent("formatchange", this, format);
    },
    ...
    print: function(map, pages, options) {
        ...
        var jsonData = Ext.apply({
            ...
            outputFormat: this.format.get("name")
        }, this.customParams);
        ...
        if(this.method === "GET") {
            ...
            Ext.Ajax.request({
                ...
                success: function(response) {
                    ...
                    this.download(url, jsonData.outputFormat);
                },
     ...
     *  :param url: ``String``
     */
    download: function(url, outputFormat) {
        if (this.fireEvent("beforedownload", this, url) !== false) {
			...
			if (outputFormat.toUpperCase()=="PDF") {
				...
				printWindow.document.write('<img src="'+url+'" onload="print(); close();"/>');
			}
        }
